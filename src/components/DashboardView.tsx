...
export default function DashboardView() {
    ...
    return (
        <div className="p-8">
            <h2 className="text-3xl font-bold text-center mb-12 text-gray-800">לוח בקרה - מצב פרויקטים בזמן אמת</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {projects.map((project) => (
                    <div key={project.id} className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 relative overflow-hidden">
                        ...
                        <div>
                            <h4 className="text-sm font-semibold text-gray-500 mb-2 uppercase tracking-wider">משתתפים רשומים:</h4>
                            {project.users && project.users.length > 0 ? (
                                <ul className="space-y-2">
                                    {project.users.map((user, index) => (
                                        <li key={index} className="flex items-center text-gray-700 bg-gray-50 px-3 py-1.5 rounded-md">
                                ...
                                <p className="text-gray-400 italic text-sm">אין נרשמים עדיין</p>
                            )}
                        </div>
                    </div>
                ))}
